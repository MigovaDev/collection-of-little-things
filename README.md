...
- **Zustand + AsyncStorage** - легковесный глобальный стейт менеджер с персистом для темы
...
- **@react-native-async-storage/async-storage** - персист состояния темы
...
│   └── useTheme.ts      # Хук работы с темами через Zustand
├── services/            # Сервисы
│   └── mmkvStorage.ts   # (удалён) раньше использовался для персиста
├── store/               # Zustand сторы
...
- **Zustand Store** - управление темой приложения через глобальный стор с персистом
